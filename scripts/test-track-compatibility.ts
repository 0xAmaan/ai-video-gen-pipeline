...
import {
  ...
  isTrackCompatible,
  getCompatibleTracks,
} from "../lib/editor/trackCompatibility";
...
console.log(
  `  Video track type: ${getTrackType(videoTrack)} (expected: video)`,
);
console.log(
  `  Audio track type: ${getTrackType(audioTrack)} (expected: audio)`,
);
console.log(
  `  Overlay track type: ${getTrackType(overlayTrack)} (expected: video)`,
);
...
console.log(
  `  Video -> Video: ${isTrackCompatible(videoTrack, videoTrack2)} (expected: true)`,
);
console.log(
  `  Video -> Overlay: ${isTrackCompatible(videoTrack, overlayTrack)} (expected: true)`,
);
console.log(
  `  Audio -> Audio: ${isTrackCompatible(audioTrack, audioTrack2)} (expected: true)`,
);
console.log(
  `  Video -> Audio: ${isTrackCompatible(videoTrack, audioTrack)} (expected: false)`,
);
console.log(
  `  Audio -> Video: ${isTrackCompatible(audioTrack, videoTrack)} (expected: false)`,
);
console.log(
  `  FX -> Video: ${isTrackCompatible(fxTrack, videoTrack)} (expected: false)`,
);
console.log();
...
console.log(
  `  Compatible tracks for video (v1): [${videoCompatible.map((t) => t.id).join(", ")}] (expected: v2, o1)`,
);
console.log(
  `  Compatible tracks for audio (a1): [${audioCompatible.map((t) => t.id).join(", ")}] (expected: a2)`,
);
console.log(
  `  Compatible tracks for fx (fx1): [${fxCompatible.map((t) => t.id).join(", ")}] (expected: empty)`,
);
console.log();
