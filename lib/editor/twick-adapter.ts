  ...
  for (const track of convertedTracks) {
    for (const clip of track.clips) {
      if (!isClipAllowedOnTrack(clip.kind, track.kind)) {
        console.warn(
          "[TwickAdapter] Rejecting incompatible clip-track pairing",
          {
            clipId: clip.id,
            clipKind: clip.kind,
            trackId: track.id,
...
const clipToElement = (clip: Clip, asset?: MediaAssetMeta): ElementJSON => {
  ...
  const duration =
    Number.isFinite(clip.duration) && clip.duration > 0 ? clip.duration : 1;
  const trimStart =
    Number.isFinite(clip.trimStart) && clip.trimStart >= 0 ? clip.trimStart : 0;
  const trimEnd =
    Number.isFinite(clip.trimEnd) && clip.trimEnd >= 0 ? clip.trimEnd : 0;
  const opacity =
  ...
  if (clip.start !== start || clip.duration !== duration) {
    console.warn(
      "[TwickAdapter] clipToElement: Invalid time values corrected",
      {
        clipId: clip.id,
...
const elementToClip = (
  ...
  const trimStart =
    Number.isFinite(props.trimStart) && props.trimStart >= 0
      ? props.trimStart
      : 0;
  const trimEnd =
    Number.isFinite(props.trimEnd) && props.trimEnd >= 0 ? props.trimEnd : 0;
  const opacity =
    Number.isFinite(props.opacity) && props.opacity >= 0 && props.opacity <= 1
      ? props.opacity
      : 1;
  const volume =
    Number.isFinite(props.volume) && props.volume >= 0 ? props.volume : 1;
  ...
  if (
    ...
    !Number.isFinite(element.e) ||
    element.s < 0 ||
    element.e <= element.s
  ) {
    console.warn(
      "[TwickAdapter] elementToClip: Invalid time values corrected",
      {
