...
export const importBrandAssets = mutation({
  args: {
    projectId: v.id("videoProjects"),
    brandAssetIds: v.array(v.id("brandAssets")),
  },
  handler: async (ctx, args) => {
    ...
    const uniqueBrandIds = [...new Set(args.brandAssetIds.map((id) => id))];
    const now = Date.now();

    for (const brandAssetId of uniqueBrandIds) {
      const brandAsset = await ctx.db.get(brandAssetId);
      if (!brandAsset || brandAsset.userId !== identity.subject) {
        continue;
      }
      ...
      await ctx.db.insert("projectAssets", {
        ...
        prominence: brandAsset.prominence,
        referenceColors: brandAsset.referenceColors,
        img2imgStrength: brandAsset.img2imgStrength,
        storageId: brandAsset.storageId,
        imageUrl: brandAsset.imageUrl,
        isActive: brandAsset.isActive ?? true,
