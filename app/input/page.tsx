...
const FOLDER_STORAGE_ID = "brand-asset-folders-v1";
...
const RawInputPage = () => {
  ...
  useEffect(() => {
    ...
    try {
      const stored = window.localStorage.getItem(FOLDER_STORAGE_ID);
      if (stored) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      window.localStorage.setItem(FOLDER_STORAGE_ID, payload);
    } catch (error) {
      if (!storageWarningShownRef.current) {
